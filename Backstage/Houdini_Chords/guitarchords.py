...
GPIO.add_event_detect(2, GPIO.FALLING, callback=Input_1, bouncetime=500) # Waiting for Button 1 to be pressed.
GPIO.add_event_detect(3, GPIO.FALLING, callback=Input_2, bouncetime=500) # Waiting for Button 2 to be pressed.
GPIO.add_event_detect(4, GPIO.FALLING, callback=Input_3, bouncetime=500) # Waiting for Button 3 to be pressed.
GPIO.add_event_detect(14, GPIO.FALLING, callback=Input_4, bouncetime=500) # Waiting for Button 1 to be pressed.
GPIO.add_event_detect(15, GPIO.FALLING, callback=Input_5, bouncetime=500) # Waiting for Button 2 to be pressed.
GPIO.add_event_detect(18, GPIO.FALLING, callback=Input_6, bouncetime=500) # Waiting for Button 3 to be pressed.
GPIO.add_event_detect(23, GPIO.FALLING, callback=Input_7, bouncetime=500) # Waiting for Button 1 to be pressed.
GPIO.add_event_detect(25, GPIO.FALLING, callback=Input_8, bouncetime=500) # Waiting for Button 1 to be pressed.
GPIO.add_event_detect(8, GPIO.FALLING, callback=Input_9, bouncetime=500) # Waiting for Button 2 to be pressed.
GPIO.add_event_detect(7, GPIO.FALLING, callback=Input_10, bouncetime=500) # Waiting for Button 3 to be pressed.
...
class MyHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == '/play-video':
            if not complete:
                with open(logging_file, 'w') as f:
                    f.write("houdini override")
                print("Override received")
                complete = True
        elif self.path == '/reset':
            print("Reset received")
            complete = False
        elif self.path == '/reboot_chords':
