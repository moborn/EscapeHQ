...
int signalPin = 13;
char Passcode[P_LENGTH+1] = "5913"; // <-- Set Passcode
...
byte rowPins[cROWS] = {3, 8, 7, 5};
byte colPins[cCOLS] = {4, 2, 6};
...
const uint8_t ARDUINO_TX = 9;    // connect to RX of MP3 Player module
...
void loop() {
  ...
  if(count == P_LENGTH){
    ...
    if(!strcmp(Key_Data, Passcode)){   
      Serial.println("Solved");
      digitalWrite(signalPin, HIGH); // <-- Disable maglock
      mp3.playStart();
      mp3.playTrackRepeat(0);
    }
